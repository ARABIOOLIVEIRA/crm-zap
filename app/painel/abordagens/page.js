...
const MODELO_PADRAO = {
  ...
  descricao: "Mensagem inicial de prospecção",
  nicho: "",
  cidade: "Uberlandia",
  mensagem: "Olá! Encontrei o trabalho da {{empresa}} em {{cidade}} e queria apresentar uma ideia simples da Lumio para empresas do segmento de {{nicho}}. Posso te mandar?",
  imagemNome: "",
  imagemDataUrl: "",
};
...
export default function PaginaAbordagens() {
  ...
  function onImagem(e) {
    ...
    if (file.size > 4 * 1024 * 1024) {
      alert("Imagem muito grande. Use até 4MB.");
      return;
    }
  ...
  async function prepararWhatsApp() {
    ...
    if (!telefone) {
      setStatus("Este lead está sem WhatsApp. Edite o cadastro antes de abordar.");
    ...
    setStatus(imagemCopiada
      ...
      : "WhatsApp aberto com texto. Se precisar da imagem, use Baixar imagem ou selecione uma imagem compatível.");
  }
  ...
  async function confirmarEnvio(criarFollowup) {
    ...
    await atualizarCamposEmpresa(atual.id, campos);
    setEmpresas((lista) => lista.map((e) => e.id === atual.id ? { ...e, ...campos } : e));
    setStatus("Envio confirmado no CRM. Avançando para o próximo lead.");
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
      <div>
        ...
        <p style={{ color: "var(--text-secondary)", marginTop: 4 }}>Prepare texto e imagem uma vez. Trabalhe um lead por vez, sem disparo automático.</p>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "minmax(320px, 420px) 1fr", gap: 16, alignItems: "start" }}>
        <section className="glass-panel" style={{ padding: 16, display: "flex", flexDirection: "column", gap: 10 }}>
          ...
          <p style={{ color: "var(--text-secondary)", fontSize: 12 }}>Variáveis: {"{{empresa}}"}, {"{{contato}}"}, {"{{nicho}}"}, {"{{cidade}}"}, {"{{bairro}}"}, {"{{responsavel}}"}</p>
        ...
        <section className="glass-panel" style={{ padding: 16, display: "flex", flexDirection: "column", gap: 12 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12 }}>
            <h3>Fila de prospecção</h3>
            <strong>{fila.length ? `${indice + 1} de ${fila.length}` : "0 de 0"}</strong>
          </div>

          {!atual ? (
            <p style={{ color: "var(--text-secondary)" }}>Selecione empresas na aba Empresas e clique em Criar abordagem, ou crie uma rodada automática com os leads da campanha atual.</p>
          ) : (
            <>
              <div style={{ border: "1px solid var(--panel-border)", borderRadius: 8, padding: 12 }}>
                <h3 style={{ marginBottom: 4 }}>{atual.nome}</h3>
                <p style={{ color: "var(--text-secondary)" }}>{atual.whatsapp || "sem WhatsApp"} · {atual.nicho || "Geral"} · {atual.cidade || "Uberlandia"}</p>
                <p style={{ color: "var(--text-secondary)" }}>Etapa: {atual.etapa_funil || atual.status || "Novo lead"} · Status abordagem: {atual.abordagem_status || "pendente"}</p>
              </div>
              ...
              <div className="glass-panel" style={{ padding: 12, display: "flex", flexDirection: "column", gap: 10 }}>
                <strong>Você enviou esta abordagem?</strong>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(150px, 1fr))", gap: 8 }}>
                  ...
                  <Botao tipo="outline" onClick={() => marcar("numero_invalido")}>Número inválido</Botao>
                  <Botao tipo="danger" onClick={() => marcar("ignorado")}>Não abordar</Botao>
                </div>
              </div>
            </>
          )}
        ...
        {conclusao && (
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              <Botao onClick={() => { setConclusao(null); window.location.assign("/painel/followups"); }}>
                Ver follow-ups
              </Botao>
